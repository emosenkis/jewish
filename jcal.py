...
def Tishrei1(metonicCycleYear, molad):
    ...
    moon) which starts that year, this routine will calculate what day will be
    the actual start of the year (Tishrei 1 or Rosh Ha-Shanah). This first day
    of the year will be the day of the molad unless one of 4 rules (called
    dehiyyot) delays it. These 4 rules can delay the start of the year by as
    much as 2 days.
    """
    tishrei1 = molad.day
    dow = molad.dayOfWeek()
...
def SdnToJewish(sdn):
    ...
    Returns:
        ...
        a JewishDate with year > 0; month in the range 1 to 13; day in the range
        1 to 30 inclusive.
    """
    ...
    if inputDay >= tishrei1:
        ...
        if inputDay >= nextTishrei1 - 177:
            ...
            # We need the length of the year to figure this out, so find Tishrei
            # 1 of this year.
...
def JewishToSdn(jdate):
    """Convert a JewishDate to serial day number (SDN).

    Zero is returned when the input date is detected as invalid. The return
    value will be > 0 for all valid dates, but there are some invalid dates that
    will return a positive value. To verify that a date is valid, convert it to
    SDN and then back and compare with the original.
    """
    # TODO(eitan): maybe raise an exception instead of returning None
    year = jdate.year
    month = jdate.month
    ...
    if year <= 0 or day <= 0 or day > 30:
        return 0
    metonicCycle, metonicYear, molad, tishrei1 = FindStartOfYear(year)
    molad.addLunarCycles(monthsInMetonicYear(metonicYear))
    nextTishrei1= Tishrei1((metonicYear + 1) % 19, molad)
    yearLength = nextTishrei1 - tishrei1
    adarsLength = 59 if jdate.isLeapYear else 29
    offset = {
        TISHREI: -1,
        CHESHVAN: 29,
        # Kislev is variable
